...
run<-function(studyId, shortRun=TRUE, enableProgress=TRUE){
  ...
  # TODO: exclude this to an external function
  ...
  for(var.alphaIterator in 1:nCases){
    ...
    for(repIterator in 1:nRepititions){
      ...
      aktSample.estimator2=S.estimator2(aktSample$Z,theta.hat)
      aktSample.estimator3=S.estimator3(aktSample$Z,theta.hat)
      ...
      for(bootIterator in 1:nBoot){
        ...
        help=S.estimator2.t(aktSample.estimator2$time,bootSample$Z,boot.theta.hat)
        help.new=S.estimator3.t(aktSample.estimator3$time,bootSample$Z,boot.theta.hat)
        W.hat.S=c(W.hat.S,sqrt(samplesize)*max(abs(help-aktSample.estimator2$surv)))
        W.hat.new=c(W.hat.new,sqrt(samplesize)*max(abs(help.new-aktSample.estimator3$surv)))
      ...
      upper.p1=aktSample.estimator2$surv+1/sqrt(samplesize)*q.alpha
      lower.p1=aktSample.estimator2$surv-1/sqrt(samplesize)*q.alpha
      scb.p1=list(time=aktSample.estimator2$time, surv=aktSample.estimator2$surv,
                  upper=upper.p1, lower=lower.p1)
      
      upper.new=aktSample.estimator3$surv+1/sqrt(samplesize)*q.alpha.new
      lower.new=aktSample.estimator3$surv-1/sqrt(samplesize)*q.alpha.new
      scb.new=list(time=aktSample.estimator3$time, surv=aktSample.estimator3$surv,
                   upper=upper.new, lower=lower.new)
      
      tmp.exp=aktSample.estimator2$surv*log(aktSample.estimator2$surv)
      tmp.bounds=1/sqrt(samplesize)*q.alpha/tmp.exp
      upper.p3=aktSample.estimator2$surv^(exp(tmp.bounds))
      lower.p3=aktSample.estimator2$surv^(exp(-tmp.bounds))
      scb.p3=list(time=aktSample.estimator2$time, surv=aktSample.estimator2$surv,
                  upper=upper.p3, lower=lower.p3)
      
      tmp.exp.new=aktSample.estimator3$surv*log(aktSample.estimator3$surv)
      tmp.bounds.new=1/sqrt(samplesize)*q.alpha.new/tmp.exp.new
      upper.new.t=aktSample.estimator3$surv^(exp(tmp.bounds.new))
      lower.new.t=aktSample.estimator3$surv^(exp(-tmp.bounds.new))
      scb.new.t=list(time=aktSample.estimator3$time, surv=aktSample.estimator3$surv,
